...
export const AlgoGreatness = () => {
  ...
  return (
    <div className="py-16 md:py-24 border-t border-white/10 mt-10 md:mt-20">
      ...
      >
        <span className="text-emerald-500 font-bold text-[10px] md:text-xs uppercase tracking-[0.3em] mb-4 inline-block">Under the Hood</span>
        <h2 className="text-3xl md:text-5xl font-bold text-white mb-4 md:mb-6 tracking-tight">How Our Algos Dominate</h2>
        <p className="text-gray-400 max-w-2xl mx-auto text-sm md:text-lg">Watch the breakdown of our proprietary trading logic and see why thousands trust IFXTrades.</p>
      </motion.div>
      ...
      <motion.div 
        ...
        className="relative max-w-5xl mx-auto mb-20 md:mb-32 group cursor-pointer px-4"
        onClick={() => alert("Video player opening...")}
      >
        <div className="absolute -inset-1 bg-gradient-to-r from-emerald-500 to-emerald-900 rounded-2xl md:rounded-[3rem] blur opacity-20 group-hover:opacity-40 transition duration-1000"></div>
        <div className="relative bg-zinc-900 border border-white/10 rounded-2xl md:rounded-[3rem] aspect-video overflow-hidden flex items-center justify-center">
          ...
          {/* Play Button */}
          <div className="relative z-10 w-16 h-16 md:w-24 md:h-24 bg-emerald-500 rounded-full flex items-center justify-center shadow-[0_0_50px_rgba(16,185,129,0.5)] group-hover:scale-110 transition-transform">
            <Play className="w-6 h-6 md:w-10 md:h-10 text-black ml-1.5 md:ml-2" />
          </div>
          
          <div className="absolute bottom-4 left-4 right-4 md:bottom-8 md:left-8 md:right-8 flex justify-between items-end">
            <div>
              <div className="text-white font-bold text-sm md:text-2xl mb-1 md:mb-2">The Neural Net Architecture</div>
              <div className="text-emerald-400 font-mono text-[10px] md:text-sm">04:28 / 12:15</div>
            </div>
            <div className="flex gap-2">
              <div className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-emerald-500 animate-pulse" />
              <div className="text-emerald-500 text-[8px] md:text-xs font-bold uppercase tracking-widest">Live Demo</div>
            </div>
          </div>
        </div>
      </motion.div>
          ...
          {reviews.map((rev, i) => (
            ...
            >
              ...
              <div className="flex gap-1 mb-4 md:mb-6">
                {[...Array(rev.rating)].map((_, j) => <Star key={j} className="w-4 h-4 md:w-5 md:h-5 text-emerald-500 fill-emerald-500" />)}
              </div>
