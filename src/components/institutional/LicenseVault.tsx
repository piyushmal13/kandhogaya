...
/**
 * LicenseVault — Secure Algo License Manager
 * Fetches real license keys from Supabase `bot_licenses` table.
 * Provides copy-to-clipboard and .EX5 download for each active license.
 */
export const LicenseVault = () => {
  ...
  const activeLicenses = dashData?.licenses || [];
  ...
  if (isLoading) {
    return (
      <div className="h-[160px] rounded-[2rem] bg-white/[0.02] animate-pulse border border-white/5" />
    );
  }
    ...
    return (
      <div className="p-6 rounded-[2rem] bg-white/[0.02] border border-white/5">
        ...
        <p className="text-gray-500 text-sm">
          You currently have no active algorithm licenses. Visit the Marketplace to acquire an institutional model.
        </p>
      </div>
    );
  }
      ...
      <div className="relative z-10">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-emerald-500/60 text-[10px] font-black uppercase tracking-widest">
        ...
        <div className="space-y-4">
          {activeLicenses.map((license) => {
            const displayKey = license.license_key || 'IFX-PENDING';
            const expiresLabel = license.expires_at
              ? new Date(license.expires_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
              : 'Perpetual';

            return (
              <div key={license.id} className="p-5 rounded-2xl bg-white/5 border border-white/10 hover:border-emerald-500/30 transition-all">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h4 className="text-white font-bold">{(license.algo_id || 'ALGO').toUpperCase()} Core</h4>
                    <p className="text-white/40 text-xs mt-1">
                      Expires: <span className="text-emerald-400">{expiresLabel}</span>
                    </p>
                  </div>
                  <button
                    onClick={() => {
                      ...
                      a.href = `data:text/plain;charset=utf-8,IFX_Institutional_License_${license.license_key}`;
                      a.download = `${license.algo_id || 'algo'}_IFX.ex5`;
                      a.click();
                    }}
