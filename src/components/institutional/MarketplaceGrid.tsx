      ...
      {products.map((product, index) => (
        ...
        >
          ...
          <motion.div 
            ...
            className={cn(
              "relative h-full overflow-hidden rounded-[2.5rem] sm:rounded-[3.5rem] bg-black/40 backdrop-blur-3xl border border-white/5 transition-all duration-700 preserve-3d",
              "hover:border-emerald-500/20 shadow-2xl",
              "flex flex-col p-6 sm:p-12 z-10"
            )}
          >
            ...
            <div className="flex justify-between items-start mb-8 sm:mb-12">
              ...
              {showPremiumBadges && product.isPremium && (
                <div className="flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-amber-500/10 border border-amber-500/20 shadow-[0_0_20px_rgba(245,158,11,0.1)]">
                  <Lock className="w-3 sm:w-3.5 h-3 sm:h-3.5 text-amber-500" />
                  <span className="text-[9px] sm:text-[10px] font-black text-amber-500 uppercase tracking-[0.1em]">Elite Tier</span>
                </div>
              )}
            </div>
            ...
            <div className="flex-1 space-y-6 sm:space-y-8">
              <h3 className="text-2xl sm:text-4xl font-black text-white leading-none uppercase tracking-tighter group-hover:text-emerald-400 transition-colors">
                {product.name}
              </h3>
              
              <p className="text-white/30 text-sm sm:text-base leading-relaxed line-clamp-3 font-medium">
              ...
              {product.type === 'algorithm' && product.performance && (
                <div className="grid grid-cols-2 gap-4 sm:gap-8 py-6 sm:py-8 border-y border-white/5 my-6 sm:my-10 bg-white/[0.01] px-4 sm:px-6 rounded-2xl sm:rounded-3xl">
                  <div className="space-y-1 sm:space-y-2">
                    <span className="text-[8px] sm:text-[9px] font-black text-white/20 uppercase tracking-[0.25em]">Alpha Rate</span>
                    <div className="text-xl sm:text-3xl font-black text-emerald-400 font-mono">
                      {product.performance.winRate}%
                    </div>
                  </div>
                  <div className="space-y-1 sm:space-y-2">
                    <span className="text-[8px] sm:text-[9px] font-black text-white/20 uppercase tracking-[0.25em]">Target CAGR</span>
            ...
            {/* Deployment Action */}
            <div className="mt-8 sm:mt-12 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
              <div className="flex flex-col">
                <span className="text-[9px] font-black text-white/20 uppercase tracking-widest mb-1 sm:mb-2">Base Allocation</span>
              ...
              <Button
                variant="elite"
                className="w-full sm:w-auto rounded-xl sm:rounded-2xl px-6 sm:px-10 py-4 sm:py-6 h-auto text-[10px] sm:text-[11px] font-black uppercase tracking-[0.2em] shadow-glow-emerald"
